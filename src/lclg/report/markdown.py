...
def render_markdown(result: PipelineResult) -> str:
    ...
    if result.comparison and result.comparison.results:
        toc.append("- [Provider Comparison](#provider-comparison)")
    toc += [
    ...
    lines += [
        "---",
        "",
        f"_Generated by [langchain-gateway-demo](https://github.com/Axemere-LLC/langchain-gateway-demo)"
        f" · Powered by [Axemere Gateway](https://axemere.ai) · Run `{result.run_id}`_",
        "",
    ]
